...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(ecrio.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(ecrio.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::ecrio_token_abi().data()).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(ecrio.token));
         set_code( N(ecrio.token), contracts::ecrio_token_wasm() );

         auto result = push_action(N(ecrio.token), N(create), mutable_variant_object()
                 ("issuer",       ecrio_token)
         ...
         result = push_action(N(ecrio.token), N(issue), mutable_variant_object()
                 ("to",       ecrio_token)
      ...
      static const std::string ecrio_token;
};

const std::string currency_tester::ecrio_token = name(N(ecrio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(ecrio.token), expected.get_symbol(), N(ecrio.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(ecrio.token), N(transfer), mutable_variant_object()
         ("from", ecrio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(ecrio.token), N(transfer), mutable_variant_object()
                                    ("from", ecrio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(ecrio_token, N(alice), "100.0000 CUR");
